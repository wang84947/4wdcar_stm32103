...
{
    struct rt_device_pin_mode rtd;
    struct rt_device_pin_mode err;
} canled[] =
{
#ifdef USING_BXCAN1
    {
...
static struct rt_can_filter_item filter1item[4] =
{
...
static struct rt_can_filter_item filter2item[4] =
{
...
static struct rt_can_filter_config filter1 =
{
    4,
    1,
    filter1item,
};
static struct rt_can_filter_config filter2 =
{
    4,
    1,
    filter2item,
...
extern rt_uint32_t adc_vol, adc_value;
void rt_can_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        msg.id=0x23;
			msg.len=8;
			//rt_memcpy(msg.data,&adc_value,4);
			//rt_memcpy(msg.data+4,&adc_vol,4);
			msg.data[0]=(adc_value>>8)&0xff;
			msg.data[1]=adc_value&0xff;
			msg.data[2]=(adc_vol>>8)&0xff;
			msg.data[3]=(adc_vol)&0xff;			
...
//INIT_APP_EXPORT(rt_can_app_init,can test);
